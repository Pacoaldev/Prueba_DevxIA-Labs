...
@Component({
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          <div class="form-group">
            ...
            @if (form.get('email')?.touched && form.get('email')?.invalid) {
              <div class="error">Correo electrónico no válido.</div>
            }
          </div>
          ...
          <div class="form-group">
            ...
            @if (form.get('password')?.touched && form.get('password')?.invalid) {
              <div class="error">La contraseña debe tener al menos 6 caracteres.</div>
            }
          </div>
  ...
  `,
  styles: [`
    .login-container { display: flex; justify-content: center; align-items: center; min-height: 100vh; background: #f1f5f9; font-family: system-ui, -apple-system, sans-serif; }
    .login-card { background: white; padding: 2.5rem; border-radius: 12px; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.08); width: 100%; max-width: 420px; }
    .header { text-align: center; margin-bottom: 2rem; }
    .header h2 { margin: 0 0 0.5rem; color: #0f172a; font-size: 1.75rem; }
